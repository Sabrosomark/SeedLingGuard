...
def current_DHT11_temperature(sensor_instance):
    ...
    return (result.temperature * 9/5) + 15  # Correctly return the temperature without additional conversion
...
def open_left_blind():
    ...
    sleep(1.5)  # Motor runs for 1.5 seconds to open left blind
    GPIO.output(in1, GPIO.LOW)
...
def close_left_blind():
    ...
    p.ChangeDutyCycle(100)
    sleep(1.4)  # Motor runs for 1.4 seconds to close left blind
    GPIO.output(in2, GPIO.LOW)
...
def open_right_blind():
    ...
    sleep(1.5)  # Motor runs for 1.5 seconds to open right blind
    GPIO.output(in3, GPIO.LOW)
...
def close_right_blind():
    ...
    p2.ChangeDutyCycle(100)
    sleep(1.4)  # Motor runs for 1.4 seconds to close right blind
    GPIO.output(in4, GPIO.LOW)
...
def read_sensors():
    try:
        ...
        avg_tds_voltage2 = average_tds_voltage(5, chan_tds2)
...
def sensor_data():
    ...
    # Adjust pump statuses based on sensor readings
    if soil_moisture_level1 == 'WET' and soil_moisture_level2 == 'WET':
        water_pump_status = "IDLE"
    elif soil_moisture_level1 == 'DRY' or soil_moisture_level2 == 'DRY' or tds_level1 == 'High Salinity':
...
def control_blinds(temp1, temp2):
    global blinds_status_left, blinds_status_right

    # Determine desired state based on temperature readings
    desired_state_left = "CLOSE" if temp1 > TEMP_THRESHOLD_CLOSE else "OPEN"
    desired_state_right = "CLOSE" if temp2 > TEMP_THRESHOLD_CLOSE else "OPEN"

    # Control the left blind
    if blinds_status_left != desired_state_left:
        if desired_state_left == "CLOSE":
            close_left_blind()
            blinds_status_left = "CLOSE"
        elif desired_state_left == "OPEN":
            if blinds_status_left == "CLOSE" and temp1 < TEMP_THRESHOLD_OPEN:
                open_left_blind()
                blinds_status_left = "OPEN"

    # Control the right blind
    if blinds_status_right != desired_state_right:
        if desired_state_right == "CLOSE":
            close_right_blind()
            blinds_status_right = "CLOSE"
        elif desired_state_right == "OPEN":
            if blinds_status_right == "CLOSE" and temp2 < TEMP_THRESHOLD_OPEN:
                open_right_blind()
